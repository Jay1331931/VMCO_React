  ...
  useEffect(() => {
    ...
    const initializeMap = async () => {
      mapInstance = new maplibregl.Map({
        ...
        center:
          initialLat && initialLng ? [initialLng, initialLat] : defaultCenter,
        zoom: zoom,
      });
...
function CustomersDetails() {
  ...
  const transformCustomerData = async (customer, customerContacts) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.log(
          `Workflow check result for customer ${customer?.id}:`,
          isAppMode
        );
      } else {
        console.log(
          `Workflow check failed for customer ${customer?.id}:`,
          res.status
        );
      }
    } catch (err) {
  ...
  const fetchCustomerContacts = async (customerId, customer) => {
    try {
      ...
      if (result.status === "Ok") {
        const transformedData = await transformCustomerData(
          customer,
          result.data
        );
  ...
  const fetchApprovedCustomer = async (transformedCustomer) => {
    ...
    try {
      ...
      if (transformedCustomer?.workflowData?.updates?.methodDetails) {
        transformedCustomer.workflowData = transformWorkflowData(
  ...
  const [companyType, setCompanyType] = useState(
    transformedCustomer?.companyType?.toLowerCase() || ""
  );
  ...
  const tabs = useMemo(() => {
    ...
    if (customerFormMode === "custDetailsAdd") {
      ...
      if (
        ...
        transformedCustomer?.customerStatus === "pending"
      ) {
        return allTabs.filter(
          (tab) => tab !== "Branches" && tab !== "Products & MoQ"
        );
      } else {
        return allTabs;
      }
    ...
    if (customerFormMode === "custDetailsEdit") {
      if (
        transformedCustomer?.customerStatus === "approved" &&
        transformedCustomer?.module === "branch"
      ) {
        return allTabs;
      } else {
        return allTabs.filter(
    ...
    Object.keys(formsByTab).forEach((tab) => {
      ...
      fields.forEach((field) => {
        if (!(field.name in allData)) {
          allData[field.name] =
            tabData[field.name] || (field.type === "checkbox" ? [] : "");
        }
      });
    });
    return allData;
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "companyType") {
      ...
      formsByTab["Documents"] = getDocumentsForm(t, value.toLowerCase())[
        "Documents"
      ];
      formDataByTab["Documents"] = getBusinessDetailsFormData(
        t,
        customer,
        value.toLowerCase()
      )["Documents"];
  ...
  const handleSave = async (action) => {
    switch (action) {
      ...
      case "submit":
        if (activeTab === "Contact Details") {
          ...
          if (
            (formData.financeHeadEmail &&
              formData.financeHeadEmail === formData.primaryContactEmail) ||
            (formData.purchasingHeadEmail &&
    ...
    const paymentMethodPayload = (() => {
      ...
      const hasPaymentChanges = paymentDetailFields.some((field) => {
        ...
        const oldVal =
          transformedCustomer[field] ??
          transformedCustomer[field.split(".")[0]]?.[field.split(".")[1]];
      ...
      if (newValue !== oldValue || action === "submit") {
        if (contactDetailFields.includes(fieldName)) {
          ...
          if (fieldName === "creditLimit" || fieldName === "creditPeriod" || fieldName === "creditLimitDar" || fieldName === "creditPeriodDar" || fieldName === "creditLimitNaqi" || fieldName === "creditPeriodNaqi" || fieldName === "creditLimitGmtc" || fieldName === "creditPeriodGmtc" || fieldName === "creditLimitVmco" || fieldName === "creditPeriodVmco" || fieldName === "creditLimitShc" || fieldName === "creditPeriodShc") {
            ...
            paymentMethodPayload.method_details[fieldName].isAllowed =
              newValue?.isAllowed;
          } else {
          ...
          if (
            fieldName !== "undefined" &&
            fieldName !== "businessHeadSameAsPrimary"
          )
            customerPayload[fieldName] = newValue;
          customerPayload["customerStatus"] = (
            formData.customerStatus || customerData.customerStatus
          ).toLowerCase();
          if (
    ...
    try {
      ...
      if (
        !(transformedCustomer?.customerStatus === "new") &&
        paymentChangesIsThere &&
        Object.keys(paymentMethodPayload.method_details).length > 0
      ) {
  ...
  const fetchDropdownOptions = async () => {
    ...
    const dropdownFields = formsByTab[activeTab].filter(
      (field) =>
        field.type === "dropdown" || field.type === "conditionalDropdown"
    );
  ...
  const fetchEmployeeDropdownOptions = async () => {
    const options = {};
    const dropdownFields = formsByTab[activeTab].filter(
      (field) => field.type === "dropdownObject"
    );
    // console.log("===============Dropdown Fields", dropdownFields)
    // for (const field of dropdownFields) {
  ...
  const handleDialogSubmit = async (comment) => {
    ...
    if (transformedCustomer?.name === "payment methods") {
      ...
      if (
        updates.creditLimit ||
        updates.creditPeriod ||
        updates.creditBalance
      ) {
        methodDetails.credit = methodDetails.credit || {};
        methodDetails.credit.limit =
          updates.creditLimit || methodDetails.credit?.limit;
        methodDetails.credit.period =
          updates.creditPeriod || methodDetails.credit?.period;
        methodDetails.credit.balance =
          updates.creditBalance || methodDetails.credit?.balance;
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/workflow-instance/id/${transformedCustomer.workflowInstanceId}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleSubmit = async (action) => {
    ...
    if (action === "submit") {
      ...
      const areaSalesManager =
        (await getManagerFromEmployees(formData?.region?.toLowerCase())) || "";
      ...
      try {
        ...
        if (
          ...
          Object.keys(paymentMethodPayload).length > 0
        ) {
          await fetch(`${API_BASE_URL}/payment-method/id/${customer.id}`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
  ...
  const handleCheckboxChange = (e, fieldName) => {
    ...
    setFormData((prev) => {
      ...
      if (fieldName === "businessHeadSameAsPrimary") {
        ...
        setChangedFields((prev) =>
          new Set(prev).add("businessHeadDesignation")
        );
  ...
  const renderHeaderWithLinks = (label) => {
    ...
    const elements = label
      .split(new RegExp(`(${Object.keys(linkMap).join("|")})`))
      .map((part, index) => {
        if (linkMap[part]) {
          return (
            <a
  ...
  const shouldShowDiv =
    (transformedCustomer?.isApprovalMode || customer?.isApprovalMode) &&
    customerFormMode === "custDetailsAdd" &&
    (transformedCustomer.customerStatus !== "new" ||
          ...
          <>
            <div>
              <CommentPopup
                ...
                externalComments={
                  transformedCustomer.approvalHistory
                    ? transformedCustomer.approvalHistory
                    : []
                }
                isVisible={true}
