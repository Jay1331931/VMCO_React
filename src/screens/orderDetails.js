...
function OrderDetails() {
  ...
  if (formData.id && isEditMode) {
    ...
    fieldsToUpdate.forEach(field => {      if (formData[field] !== undefined && formData[field] !== null) {
  ...
  while (attempt < maxAttempts) {
    ...
    const payload = {
      ...
      paymentMethod: formData.category && formData.category.toLowerCase() === 'vmco machines' 
        ? 'Pre Payment' 
        : (selectedMethod || formData.paymentMethod || ''),      paymentPercentage: formData.paymentPercentage ?
...
const handleSelectBranch = (branch) => {
  ...
  setFormData(prev => ({
    ...
    branchRegion: branch.region || branch.branchRegion || '', // Set branchRegion
  }));
  ...
  try {
    ...
    if (approvalAction === 'approve' && fromApproval && formData.status && formData.status.toLowerCase() === 'pending') {
      try {
        ...
        const patchPayload = {
          totalAmount: formData.totalAmount,          paymentPercentage: formData.paymentPercentage ?
            (formData.paymentPercentage === '100%' ? '100.00' :
              formData.paymentPercentage === '30%' ? '30.00' : '0.00') : '0.00'
        };
                    ...
                    <div className="order-details-field">
                      ...
                      {formMode === 'add' ? (
                        <select
                          ...
                          onChange={handleInputChange}
                          disabled={formData.category !== 'VMCO Machines'}
                        >
