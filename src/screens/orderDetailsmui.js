...
function OrderDetails() {
    ...
    const productColumns = [
        ...
        },
        {
            ...
            renderCell: (params) => {
                const row = params.row;
                const idx = formData.products.findIndex(p => (p.id || p.productid) === (row.id || row.productid));
        ...
        ...(isE('deleteCol') ? [{
            ...
            renderCell: (params) => {
                ...
                return isV('deleteButton') && isE('deleteCol') && (
                    <button
    ...
    const visibleProductColumns = productColumns.filter(col => col.include !== false);
                                ...
                                {isV('products') && (
                                    <>
                                        ...
                                        {((formMode !== 'add') || ((formData.products || []).length > 0)) && (
                                            <div className="order-products-section" style={{ boxShadow: '0 0 10px rgba(0,0,0,0.1)', padding: '16px', borderRadius: '8px' }}>
                                                <DataGrid
                                                    apiRef={gridApiRef}
                    ...
                    {isV('orderFooter') && (
                        <div className="order-details-footer">
                            ...
                            <div className="" style={{ display: "flex", gap: "10px" }}>
                                ...
                                {isV('btnPay') && isE('btnPay') && formData?.paymentMethod?.toLowerCase() != "cash on delivery" && ((formData?.paymentStatus?.toLowerCase() !== "paid" || formData?.paymentStatus?.toLowerCase() !== "credited" )  || formData?.paymentStatus?.toLowerCase() !== 'credited')
                                ...
                                {isV('btnSendLink') && isE('btnSendLink') && formData?.paymentMethod?.toLowerCase() != "cash on delivery" && ((formData?.paymentStatus?.toLowerCase() !== "paid" || formData?.paymentStatus?.toLowerCase() !== "credited" )  || formData?.paymentStatus?.toLowerCase() !== 'credited')
                                    && (formData?.status?.toLowerCase() === 'approved' || (formData?.status?.toLowerCase() === 'open'
                                        && (formData?.entity.toLowerCase() === Constants.ENTITY.DAR.toLowerCase() || formData?.entity.toLowerCase() === Constants.ENTITY.GMTC.toLowerCase() || formData?.entity.toLowerCase() === Constants.ENTITY.SHC.toLowerCase())) ||
                                        (formData?.status?.toLowerCase() === 'pending' && (formData.entity.toLowerCase() === Constants.ENTITY.DAR.toLowerCase() || formData?.entity.toLowerCase() === Constants.ENTITY.NAQI.toLowerCase()))) && (
